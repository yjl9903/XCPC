...
int n, m, c[maxn], f[maxn][maxn], pref[2][maxn], g[maxn][maxn], preg[2][maxn];
char s[maxn];
...
int main() {
  ...
  for (int i = 1; i <= m; i++) {
    for (int u = 1; u <= acam::sz; u++) {
      for (int c = 0; c < 26; c++) {
        int nx = acam::ch[u][c];
        f[i][nx] = add(f[i][nx], add(f[i - 1][u], mul(g[i - 1][u], acam::val[nx])));
        g[i][nx] = add(g[i][nx], g[i - 1][u]);
      }
      f[i][u] = add(f[i][u], pref[i % 2][u]);
      g[i][u] = add(g[i][u], preg[i % 2][u]);
    }
    for (int u = 1; u <= acam::sz; u++) {
      dbg(i, f[i][u]);
      pref[i % 2][u] = add(pref[i % 2][u], f[i][u]);
      preg[i % 2][u] = add(preg[i % 2][u], g[i][u]);
    }
  }
  int ans = 0;
  for (int u = 1; u <= acam::sz; u++) {
