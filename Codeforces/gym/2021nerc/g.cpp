...
int main() {
  ...
  double ans = 0.0;
  Point sun(xx[n], yy[n] + H);
  Line lsun(sun, Point(xx[n], yy[n]));
  bool up = false;
  for (int i = n - 1; i >= 1; i--) {
    Point pre(xx[i], yy[i]);
    Point nxt(xx[i + 1], yy[i + 1]);
    Line cur(pre, nxt);
    if (isParallel(lsun, cur)) {
      int dir1 = clockwise(sun, nxt, lsun[1]);
      int dir2 = clockwise(sun, pre, lsun[1]);
      if (dir1 >= 0 && dir2 >= 0) {
        ans += pre.distance(nxt);
      }
      // if (!up) {
      //   ans += pre.distance(nxt);
      // } else {
      ...
      Point its = getLineIntersection(lsun, cur);
      if (its != nxt && onSegment(cur, its)) {
        ans += its.distance(pre);
      } else {
        int dir1 = clockwise(sun, nxt, lsun[1]);
        int dir2 = clockwise(sun, pre, lsun[1]);
        if (dir1 >= 0 && dir2 >= 0) {
          // if (!up) {
          //   int dir = clockwise(sun, pre, nxt);
          //   assert(dir != 0);
          //   if (dir == 1) {
          //     ans += pre.distance(nxt);
          //   }
    ...
    if (clockwise(sun, pre, lsun[1]) == 1) {
      lsun = Line(sun, pre);
    }
    dbg(ans, lsun[1].x, lsun[1].y);
  }
  printf("%.10lf\n", ans);
  return 0;
}