...
int n, node[maxn], tin[maxn], tout[maxn], tot;
char s[maxn];
...
namespace pam {
    ...
    int pos[maxn], dif[maxn], slink[maxn];
    char s[maxn];
    int node(int l) {
        sz++; ms(ch[sz], 0);
        len[sz] = l; 
        fail[sz] = dif[sz] = slink[sz] = 0;
        return sz;
    }
    void clear() {
        sz = -1; last = 0;
...
struct SegTree {
    ...
    int a[maxn << 2];
    void build(int l = 1, int r = pam::sz + 1, int rt = 1) {
        a[rt] = 0;
        if (l == r) return ;
        int m = (l + r) / 2;
...
int main() {
    ...
    while (T--) {
        ...
        tot = 0;
        g.build();
        for (int i = 0; i <= pam::sz; i++) edge[i].clear();
        for (int i = 0; i <= n; i++) que[i].clear(), f.a[i] = 0;
