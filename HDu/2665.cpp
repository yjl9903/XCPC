...
int main(){
    int T; scanf("%d", &T);
    while (T--){
        scanf("%d%d", &n, &q); h.clear(); cnt = 0;
        for (int i = 1; i <= n; i++) scanf("%d", &a[i]), h.push_back(a[i]);
        sort(h.begin(), h.end());
        vector<int>::iterator it = unique(h.begin(), h.end());
        h.resize(distance(h.begin(), it));
        build(1, h.size(), root[0]);
        for (int i = 1; i <= n; i++) a[i] = lower_bound(h.begin(), h.end(), a[i]) - h.begin() + 1;
        for (int i = 1; i <= n; i++) update(a[i], 1, root[i - 1], 1, h.size(), root[i]);
        while (q--){
