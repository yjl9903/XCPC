...
int main() {
  ...
  for (int i = 1; i <= k; i++) {
    a[i + i - 1].read();
    a[i + i].read();
    // double dis = a[i + i].distance(i + i - 1);
    // edge[i + i - 1].emplace_back(i + i, dis);
    // edge[i + i].emplace_back(i + i - 1, dis);
  }
  a[k + k + 1].read();
  a[k + k + 2].read();
  for (int i = 1; i <= k + k + 2; i++) {
    for (int j = 1; j < i; j++) {
      Line l(a[j], a[i]);
      bool flag = true;
      for (int p = 1; p <= k && flag; p++) {
        if (p + p - 1 == i || p + p - 1 == j) continue;
        if (p + p == i || p + p == j) continue;
        flag &= !isSegmentIntersect(l, Line(a[p + p - 1], a[p + p]));
      }
      if (flag) {
        double dis = a[j].distance(a[i]);
        edge[i].emplace_back(j, dis);
        edge[j].emplace_back(i, dis);
        dbg(j, i, dis);
      }
    }
  }
